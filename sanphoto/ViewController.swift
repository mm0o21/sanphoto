...
class ViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    var adr: String!
    var userLocation : CLLocationCoordinate2D? = CLLocationCoordinate2DMake(35.68124, 139.76672)
    ...
    override func viewDidLoad() {
        ...
        makeMap()
        print("ああああ", mapView.userLocation.coordinate)
    }
    
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        self.userLocation = manager.location?.coordinate
        mapView.region.center = manager.location!.coordinate
        //print("test")
    }
...
//    }
    ...
    @IBAction func addPin(_ sender: UIButton){
        ...
        //ピンを生成
        let pin = Pin()
        pin.date = Date().description
        let annotation = MyAnnotation(pin: pin, coordinate: coordinate)
        ...
        coordinatesArray.append(["name":"", "lat": mapView.userLocation.coordinate.latitude, "lon": mapView.userLocation.coordinate.longitude])
        ...
        self.performSegue(withIdentifier: "next", sender: pin)
...
//            let lon = location.coordinate.longitude
    ...
    func makeMap() {
        //マップの表示域を設定
        let coordinate = self.userLocation!
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        ...
        if let annotation = view.annotation{
            self.performSegue(withIdentifier: "next", sender: (annotation as! MyAnnotation).pin)
        }
    }
}
